...
countries_centroids <- readr::read_csv('/home/justyna/Projects/Rscripts/TidyTuesday/2020/2020-02-18/countries_centroids.csv')
...
plot <-ggplot() + 
  ...
  geom_polygon(data = meat_consumption_by_region, aes(x = long, y = lat, group = group, fill = consumption), colour = "white") + 
  scale_fill_gradientn(colours = palette) + 
  geom_text(data = centroids_top10, x = centroids_top10$long_cen, y = centroids_top10$lat_cen, label = centroids_top10$co2_emmission, vjust = 2, size = 3, fontface = "italic", color = text_color, family = "Verdana") + 
  geom_text(data = centroids_top10, x = centroids_top10$long_cen, y = centroids_top10$lat_cen, label = centroids_top10$region, size = 4, fontface = 4, color = text_color, family = "Verdana") +
  theme_void() + 
  theme(
